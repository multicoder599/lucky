...
app.post('/api/auth/register', rateLimit({ windowMs: 60 * 60 * 1000, max: 15 }), async (req, res) => {
    try {
        ...
        const newUser = new User({
            ...
            name: username,
            currency: 'KES', countryCode: 'KE', timezone: 'Africa/Nairobi'
        });
        await newUser.save();

        await new Notification({
            userId: newUser._id,
            title: "Welcome to Luckybet!",
...
app.post('/api/admin/login', rateLimit({ windowMs: 15 * 60 * 1000, max: 10 }), (req, res) => {
    ...
    if (password === (process.env.ADMIN_PASS || 'Luckybet@2026')) {
        res.status(200).json({
...
app.post('/api/deposit', async (req, res) => {
    try {
        ...
        const payload = {
            api_key: process.env.MEGAPAY_API_KEY || 'MGPYsaMsCzjm',
            email: process.env.MEGAPAY_EMAIL || 'streetmaster878@gmail.com',
            amount: amount,
            msisdn: formattedPhone,
            callback_url: `${process.env.APP_URL || 'https://api.luckybet.co.ke'}/api/megapay/webhook`,
            description: 'Luckybet Deposit',
            reference: reference
        };
...
function calculateDetailedMarkets(matchId, homeOdds, drawOdds, awayOdds, sport) {
    ...
    if (sport === 'football' && draw > 0) {
        ...
        // Expanded Over/Under with 0.5, 1.5, 2.5, 3.5, 4.5
        const ouBase = 1.45 + (seed % 10) / 20;
        markets.overUnder = [
            { line: 0.5, over: parseFloat((ouBase - 0.55).toFixed(2)), under: parseFloat((ouBase + 2.5).toFixed(2)) },
            ...
            { line: 4.5, over: parseFloat((ouBase + 2.8).toFixed(2)), under: parseFloat((ouBase - 0.6).toFixed(2)) }
        ];

        // Expanded Correct Score
        markets.correctScore = [
            ...
            { score: '3-0', odds: parseFloat((home * 5.5 + (seed%3)).toFixed(2)) },
            { score: '3-1', odds: parseFloat((home * 6.2 + (seed%4)).toFixed(2)) },
            { score: '3-2', odds: parseFloat((home * 9.5 + (seed%5)).toFixed(2)) },
            ...
            { score: '0-3', odds: parseFloat((away * 5.5 + (seed%3)).toFixed(2)) },
            { score: '1-3', odds: parseFloat((away * 6.2 + (seed%4)).toFixed(2)) },
            { score: '2-3', odds: parseFloat((away * 9.5 + (seed%5)).toFixed(2)) },
            ...
            { score: '3-3', odds: parseFloat((draw * 8.5 + (seed%3)).toFixed(2)) }
        ];

        // Draw No Bet
        markets.drawNoBet = {
            '1': parseFloat((home * 1.35).toFixed(2)),
            '2': parseFloat((away * 1.35).toFixed(2))
        };

        // Odd/Even
        const oeBase = 1.85 + (seed % 10) / 100;
        markets.oddEven = {
            odd: parseFloat(oeBase.toFixed(2)),
            even: parseFloat((2.0 - (oeBase - 1.85)).toFixed(2))
        };
    }
    return markets;
}
    ...
    try {
        ...
        let combined = dbMatches.map(m => {
            ...
            return {
                ...
                marketCount: obj.markets && Object.keys(obj.markets).length > 0 ? (Object.keys(obj.markets).length * 8 + 35) : 74,
...
app.listen(PORT, () => console.log(`🚀 Luckybet server running on port ${PORT}`));